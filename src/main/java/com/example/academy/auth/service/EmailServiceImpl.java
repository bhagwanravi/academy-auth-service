...
public class EmailServiceImpl implements EmailService {
    ...
    public void sendAdminRegistrationNotification(String superAdminEmail, String adminName, String adminEmail, String tenantId) {
        try {
            ...
            helper.setFrom(fromEmail); // Keep authenticated email as From
            helper.setReplyTo(adminEmail); // Set admin's email as Reply-To
            ...
            log.info("Admin registration notification sent to super admin: {} from admin: {}", superAdminEmail, adminEmail);
    ...
    @Override
    public void sendAdminApprovalRequest(String superAdminEmail, String adminName, String adminEmail, 
        ...
        try {
            ...
            helper.setTo(superAdminEmail);
            helper.setSubject("Admin Approval Request - " + adminName);
            helper.setFrom(fromEmail); // Keep authenticated email as From
            helper.setReplyTo(adminEmail); // Set admin's email as Reply-To
            ...
            model.put("adminEmail", adminEmail);
            model.put("tenantId", tenantId);
            model.put("approvalLink", approvalLink);
            model.put("rejectionLink", rejectionLink);
            
            String content = processTemplate("admin-approval-request.ftl", model);
            helper.setText(content, true);
            
            mailSender.send(message);
            log.info("Admin approval request sent to super admin: {} from admin: {}", superAdminEmail, adminEmail);
        } catch (MessagingException | IOException | TemplateException e) {
    ...
    @Override
    public void sendAdminApprovalConfirmation(String adminEmail, String status) {
        try {
            MimeMessage message = mailSender.createMimeMessage();
            MimeMessageHelper helper = new MimeMessageHelper(message, true, "UTF-8");
            
            helper.setTo(adminEmail);
            helper.setSubject("Admin Registration " + status);
            helper.setFrom(fromEmail);
            ...
            String templateName = status.equalsIgnoreCase("APPROVED") ? 
                "admin-approval-confirmed.ftl" : "admin-rejection-confirmed.ftl";
            String content = processTemplate(templateName, model);
            helper.setText(content, true);
            
            mailSender.send(message);
            log.info("Admin approval confirmation sent to admin: {}", adminEmail);
        } catch (MessagingException | IOException | TemplateException e) {
            log.error("Failed to send admin approval confirmation", e);
            throw new RuntimeException("Failed to send email notification", e);
        }
    }

    @Override
    public void sendAdminApprovalNotification(String superAdminEmail, String adminEmail, String status) {
        try {
            MimeMessage message = mailSender.createMimeMessage();
            MimeMessageHelper helper = new MimeMessageHelper(message, true, "UTF-8");
            
            helper.setTo(superAdminEmail);
            helper.setSubject("Admin Registration " + status + " - " + adminEmail);
            helper.setFrom(fromEmail);
            ...
            String content = processTemplate("admin-approval-status-notification.ftl", model);
            helper.setText(content, true);
            
            mailSender.send(message);
            log.info("Admin approval status notification sent to super admin: {}", superAdminEmail);
        } catch (MessagingException | IOException | TemplateException e) {
            log.error("Failed to send admin approval status notification", e);
            throw new RuntimeException("Failed to send email notification", e);
        }
    }
